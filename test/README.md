...
| `growth-pace.check.ts` | `growthPace.ts` — skill-point pace (incl. catch-up and non-bottleneck cases), the balanced solver, projection (incl. the on-curve invariant), overall rating and the @25 lookups, 28 assertions | 2026-09-25, all pass |
| `squad-rank.check.ts` | `squadRank.ts` — rank at position, neighbours and gaps, ties, the subject's cached copy ignored, ordinals, 9 assertions | 2026-09-25, all pass |
